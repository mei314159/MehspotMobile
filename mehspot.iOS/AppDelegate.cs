...
namespace Mehspot.iOS
{
	...
	public class AppDelegate : UIApplicationDelegate
	{
		...
		void ProcessNotification(NSDictionary userInfo, bool quiet = true)
		{
			...
			foreach (var c in tabBarController.ViewControllers)
			{
				...
				{
					if (OpenMessaging(notification, rootController, tabBarController, quiet))
					{
						break;
					}
				}
				...
				{
					if (OpenGroupMessaging(notification, rootController, tabBarController, quiet))
					{
						break;
					}
				}
		...
		private bool OpenMessaging(PushNotification notification, UINavigationController rootController, UITabBarController tabBarController, bool quiet)
		{
			if (rootController.TopViewController is MessageBoardViewController)
			{
				...
				if (quiet && messageBoardViewController.IsViewLoaded)
				{
					messageBoardViewController.UpdateList(notification.Message);
				}
				...
				if (rootController.VisibleViewController is MessagingViewController)
				{
					...
					messagingViewController.AddMessageBubbleToEnd(notification.Message);
					return true;
				}
				else if (!quiet)
				{
		...
		private bool OpenGroupMessaging(PushNotification notification, UINavigationController rootController, UITabBarController tabBarController, bool quiet)
		{
			if (rootController.TopViewController is GroupsListViewController)
			{
				var groupsListController = ((GroupsListViewController)rootController.TopViewController);
				if (quiet && groupsListController.IsViewLoaded)
				{
					groupsListController.UpdateList(notification.GroupMessage);
				}
				...
				if (rootController.VisibleViewController is GroupMessagingViewController)
				{
					...
					messagingViewController.AddMessageBubbleToEnd(notification.GroupMessage);
					return true;
				}

				if (!quiet)
				{
					groupsListController.ShowMessagesFromGroup(notification.GroupMessage);
					return true;
				}
